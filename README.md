...
- 115网盘：多账号、扫码自动登录、基本文件管理、批量重命名、校验秒传、离线下载、在线解压（可输入密码）、直链预览，以及与本地窗口之间的拖放和剪贴板传输。
...
支持预览的文本、图片和视频可双击打开，由浏览器通过115直链加载文件内容，不经 FileButler 中转；可重新获取或复制直链。浏览器不支持预览的类型不会强行作为文本打开。实际加载仍受直链有效期、115访问限制和浏览器支持情况影响。
