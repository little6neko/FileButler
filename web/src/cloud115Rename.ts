...
export function cloudPowerRenameClient(parentId: string): PowerRenameClient {
  let generation = 0;
  ...
  return {
    async renamePreview(request) {
      ...
      ready = null;
      const plan = await cloudCall<Awaited<ReturnType<typeof api.renamePreview>> & { previewToken: string }>("power.preview", { parentId, ids: request.paths, options: { ...request.options, readMetadata: false } });
    ...
    async renameCreateJob(request) {
      ...
      const { token } = ready;
      ready = null;
      return cloudCall<{ id: string }>("power.submit", { parentId, previewToken: token });
    },
  };
}

export function cloudSuperRenameClient(parentId: string): SuperRenameClient {
  ...
  return {
    ...
    async superRenamePreview() {
      const current = ++generation;
      groupVersions.clear();
      const inventory = await cloudCall<SuperRenameInventory & { revision: string }>("super.preview", { parentId });
    ...
    async superRenameGroupPreview(request) {
      ...
      const group = await cloudCall<SuperRenameInventoryGroup & { revision: string }>("super.group", { parentId, path: request.groupPath });
    ...
    async superRenameCreateJob(request) {
      ...
      const job = await cloudCall<{ id: string }>("super.submit", { parentId, paths: request.selectedPaths, revisions: needed });
