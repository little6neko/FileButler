...
export function Cloud115OfflineDialog({ target, onClose }: { target: { id: string; name: string }; onClose(): void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !busy) onClose(); }}>
    <DialogContent className="sm:max-w-lg" showCloseButton={!busy} data-no-file-drop="">
      <DialogTitle>离线下载</DialogTitle>
      <DialogDescription>保存到：{target.name}。支持磁力、ed2k、HTTP/HTTPS、FTP，每行一条。提交成功仅表示115已接受任务，不代表下载完成。</DialogDescription>
      <form className="grid min-w-0 gap-3" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        <textarea aria-label="下载链接" autoFocus rows={6} className="w-full resize-y rounded border bg-background p-2" value={text} disabled={busy} onChange={(event) => setText(event.target.value)} />
