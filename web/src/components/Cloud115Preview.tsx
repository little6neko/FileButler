...
type CloudPreviewLink = { url: string; name: string; size: number; accountId: string };
...
export function Cloud115Preview({ instance, labels, onNavigate }: { instance: CloudPreviewInstance; labels: UIStrings; onNavigate(id: string): void }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        if (descriptor) {
          const result = await cloudCall<{ accountId: string; document: TextDocument }>("preview.text", { id: entry.id, accountId: instance.accountId }, controller.signal);
          if (controller.signal.aborted) return;
          if (result.accountId !== instance.accountId) throw new Error("115账号已变化，请重新打开预览");
          session = new TextEditorSession({ id: `cloud-preview-${entry.id}`, rootId: "@115", path: entry.id, fileName: entry.name, text: descriptor });
          session.applyLoadedDocument(result.document);
        ...
        url = parsed.href;
        if (kind) setState({ key, url, loading: true, error: "" });
      } catch (error) {
        if (!controller.signal.aborted) setState({ key, url, loading: false, error: error instanceof Error ? error.message : "预览失败" });
      }
    }
  ...
  return <div className="relative flex h-full min-h-0 flex-col" data-no-file-drop data-testid="cloud-preview">
    {kind ? <div className="flex shrink-0 items-center gap-3 border-b p-2 text-sm" role="toolbar" aria-label="直链操作">
    ...
    {invalidated ? <p role="alert" className="p-4">115账号已变化，请关闭并重新打开预览</p> : <>
      ...
      {descriptor && !current ? <p role="status" className="p-3 text-sm text-muted-foreground">正在加载文本…</p> : null}
      {current?.error ? <p role="alert" className="whitespace-pre-wrap break-words p-3 text-sm text-destructive">{current.error}</p> : null}
