...
export function LocationMenu({ label, name, value, items, disabled, message, onOpen, onSelect, add }: Props) {
  return <MenuRoot onOpenChange={(open) => { if (open) onOpen?.(); }}>
    ...
    <MenuPortal><MenuPositioner align="start" sideOffset={4}><MenuPopup aria-label={label}>
      {items.map((item) => <MenuItem key={item.id} className="file-action-menu-item" onClick={() => { if (item.id !== value) onSelect(item.id); }}>
        <Check aria-hidden="true" className={`size-3.5 shrink-0${item.id === value ? "" : " invisible"}`} /><span>{item.name}</span>
      </MenuItem>)}
